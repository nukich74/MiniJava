...
#pragma message( "DO IF( DSA || DQWD )" )
	...
	if( dynamic_cast<const Tree::COpExpr*>( p.GetExp() ) ) {
		...
		if( opExpr->GetOp() == Tree::BinOp::BO_And ) {
			...
			//≈сли первый false, то дальше нет смысла прыгать
...
void CIRTreeVisitor::Visit( const Tree::CSOPStmt& p )
{
	...
	lastReturnedExp = 0;
	lastReturnedStm = 0;
	// ‘ункци€ из стандарта, с label ---, далее просто еЄ вызов с параметрами
	...
	if( var != 0 ) {
		...
		for( int i = 0; i < classVars.size(); ++i ) {
			if( classVars[i]->GetName() == p.GetName() ) {
				// обращаемс€ по соответствующему адрессу
...
void CIRTreeVisitor::Visit( const Tree::CMethodCallExpr& p )
{
	...
	//выполн€ем функцию, возращаем return
...
{	
	...
	lastReturnedExp = 0;
	//идем вглубь листа, пока не дойдем до нул€
	if( p.GetList() != 0 ) {
